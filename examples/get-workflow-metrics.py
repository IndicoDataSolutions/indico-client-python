...
print(
    ...
    + str(total_subs)
    + " for the workflow "
    + str(workflow_metric[0].workflow_id)
    + "!"
)
    ...
    [
        {
            ...
            "rejected": x.rejected_in_review,
        }
        for x in workflow_metric[0].submissions.daily
    ]
)
...
print(
    str(idx_max["date"])
    + " was the day with the most submissions: "
    + str(idx_max["submitted"])
)

""" Example 3: Retrieve only specific metrics."""
workflow_metric: List[WorkflowMetrics] = client.call(
    GetWorkflowMetrics(
        options=[WorkflowMetricsOptions.STRAIGHT_THROUGH_PROCESSING],
        start_date=datetime(2021, 6, 28),
        end_date=datetime.now(),
        workflow_ids=[workflows[0].id],
    )
